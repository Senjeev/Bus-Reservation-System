...
int check(struct node **first,int d,char name[20],int date)
{
    ...
    while(m!=NULL)
    {
        ...
        if(m10==n10)
        {
            if(n100==m100 && n1000==m1000 && m->date==date)
            {
...
case 1:
	...
	do{
        ...
        gets(name);
        do{
        printf("Which seat you want out of 1 to 50: ");
        scanf("%d",&seat);
        }while(seat<1 || seat>50);
...
case 2 :
        ...
        if(((tn-((tn)%1000))==730000) && n10<50&&n10>0 && n100<5 && n100>1
           && n1000<4 && n1000>0 )
        {
                if(set(&first,tn)==1)
                    {
